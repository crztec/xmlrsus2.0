...
import { Zap, Search, Filter, CheckCircle, XCircle, Clock, Terminal, X, Info, ChevronRight, Activity, Camera, MoreHorizontal, RotateCcw } from "lucide-react";
...
export default function ApiChecksPage() {
  ...
  const openPastLogs = (taskId: string) => {
              ...
              {isLoading ? (
                ...
                <tr key={client.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-xs font-medium text-slate-500">
                    {client.last_check_at ? formatDistanceToNow(new Date(client.last_check_at), { addSuffix: true, locale: ptBR }) : "-"}
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className="text-[13px] text-slate-600 font-medium truncate max-w-[200px] cursor-help" title={client.last_check_message}>
                        {client.last_check_message || "Sem dados."}
                      </span>
                      {(client.api_status === 'error' || client.api_status === 'offline') && client.api_last_screenshot_url && (
                         <button onClick={() => setSelectedScreenshot(client.api_last_screenshot_url || null)} className="p-1 hover:bg-rose-100 rounded text-rose-500 transition-colors" title="Ver Screenshot da Falha">
                      ...
                      {openMenuId === client.id && (
                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-blue-50 z-50 overflow-hidden animate-in zoom-in-95 duration-100 origin-top-right">
                           ...
                           {client.api_last_task_id && (
                             <button 
                               onClick={() => openPastLogs(client.api_last_task_id!)}
                             ...
                             >
                                <Terminal className="w-4 h-4" /> Ver Log Detalhado
                             </button>
                           )}
        ...
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-blue-100 flex flex-col max-h-[85vh] overflow-hidden zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-blue-50/50">
              ...
              <button onClick={() => { setShowLogs(false); setViewingTaskId(null); }} className="p-2 hover:bg-white rounded-lg transition-colors text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex justify-end bg-slate-50 gap-3">
              <button 
                onClick={() => { setShowLogs(false); setViewingTaskId(null); }}
